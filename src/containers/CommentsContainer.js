  ...
  return (
    <div className={classes.root}>
        ...
        <div className={classes.commentsContainer__questionsBox}>
         <Typography className={classes.commentsContainer__questions}>{ comments.filter((item) => item.nombre == product.nombre).length } PREGUNTAS</Typography>
        </div>
        <CommentSender 
        product={product}
        user={user}
        />
        {
          comments.filter((item) => item.nombre == product.nombre)
            .map((item,index) => (
              <Comment 
               key={index}
